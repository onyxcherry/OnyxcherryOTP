...
class User(UserMixin, db.Model):
    ...
    reset_password_value = db.relationship('ResetPasswordValue', backref='user', uselist=False)
    ...
    def get_reset_password_token(self, value, expires_in=600):
        return jwt.encode(
            {'reset_password': self.username, 'value': value, 'exp': time() + expires_in},
            current_app.config['SECRET_KEY'], algorithm='HS256').decode('utf-8')

    @staticmethod
    def delete_expired_tokens(reset_password_db):
        now_compare_time = datetime.utcnow() - timedelta(seconds=600)
        if reset_password_db.first_date and reset_password_db.first_date < now_compare_time:
            reset_password_db.first_value = None
            reset_password_db.first_date = None
            db.session.add(reset_password_db)
            db.session.commit()
        if reset_password_db.second_date and reset_password_db.second_date < now_compare_time:
    ...
    def verify_reset_password_token(token):
        try:
            jwt_decoded = jwt.decode(token, current_app.config['SECRET_KEY'], algorithms=['HS256'])
